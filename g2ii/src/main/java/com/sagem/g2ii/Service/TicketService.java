...
//
//    @Autowired
//    private JournalAuditService auditService;
//
//    // ============================================================================
//    // 1️⃣ CRÉER UN TICKET
//    // ============================================================================
...
//
//            // 💡 CORRECTION: Calcul en MINUTES pour une précision absolue
//            long minutesEcouleesPriseEnCharge = ChronoUnit.MINUTES.between(
//                    ticket.getDate(),
//                    ticket.getDatePriseEncharge()
//            );
//
//            // 💡 CORRECTION: Conversion du délai max autorisé (ex: 0.1h) en minutes (6 min)
//            double delaiMaxPriseEnChargeMinutes = ticket.getSlaAssigne().getDelaiPriseEnChargeHeure() * 60.0;
//
//            // ✅ COMPARAISON STRICTE EN MINUTES
//            boolean respectePriseEnCharge = minutesEcouleesPriseEnCharge < delaiMaxPriseEnChargeMinutes;
...
//            }
//
//            double heuresReelles = minutesEcouleesPriseEnCharge / 60.0;
//            System.out.println("   📊 SLA Prise en Charge:");
//            System.out.println("      - Temps écoulé: " + minutesEcouleesPriseEnCharge + " min (~" + String.format("%.2f", heuresReelles) + "h)");
//            System.out.println("      - Délai max autorisé: " + delaiMaxPriseEnChargeMinutes + " min (" + ticket.getSlaAssigne().getDelaiPriseEnChargeHeure() + "h)");
...
//                Double delaiMaxResolution = sla.getDelaiResolutionHeure();
//
...
//        }
//
//        System.out.println("🔍 [OBTENIR SLA] Recherche automatique par ID Catégorie: " + categorie.getIdCategorie() + " | Priorité: " + priorite);
//
//        // 💡 MODIFICATION ICI : On utilise votre requête JPQL explicite et ultra-sécurisée
//        Optional<SLA> slaApplicableOpt = slaRepository.trouverSlaUnique(categorie.getIdCategorie(), priorite);
//
//        if (slaApplicableOpt.isPresent()) {
//            SLA slaApplicable = slaApplicableOpt.get();
//            System.out.println("   ✅ SLA trouvé AUTOMATIQUEMENT: " + slaApplicable.getNomSLA() + " (ID: " + slaApplicable.getIdSLA() + ")");
//            return slaApplicable;
//        } else {
//            System.out.println("   ❌ Aucun SLA trouvé en mode automatique !");
//            System.out.println("      → ID Catégorie cherché: " + categorie.getIdCategorie());
//            System.out.println("      → Priorité cherchée: " + priorite);
//            return null;
//        }
//    }   // ============================================================================
...
/**
 * ✅ Service complet et corrigé pour la gestion des Tickets
 * Intègre un système de traçabilité et d'audit log avancé de niveau Ingénieur
 */
@Service
public class TicketService {
    ...
    public Ticket creerTicket(TicketCreationDTO dto) {
        ...
        ticket.setGroupeAssigne(groupe);
        ticket.setSlaRespecte(true);
        ticket.setSlaPriseEnChargeRespecte(null);
        ticket.setSlaResolutionRespecte(null);
        ...
        // 🛡️ AUDIT LOG : Création de Ticket
        String jsonApres = String.format("{\"titre\":\"%s\",\"priorite\":\"%s\",\"categorie\":\"%s\",\"groupe\":\"%s\"}",
                ticketSauvegarde.getTitre(), prioriteAssignee.name(), categorie.getNomCategorie(), groupe.getNomGroupes());

        auditService.enregistrerLogAvance(
                ModuleAudit.TICKET, ActionAudit.CREATE_TICKET, "Ticket", ticketSauvegarde.getIdTicket(),
                "Création initiale du ticket Réf: " + ticketSauvegarde.getReference() + " - Titre: " + ticketSauvegarde.getTitre(),
                null, jsonApres, NiveauAudit.INFO, true, demandeur
        );
    ...
    public Ticket demarrerTicket(Long idTicket, Long idUtilisateur) {
        ...
        if (ticket.getSlaAssigne() != null) {
            long minutesEcouleesPriseEnCharge = ChronoUnit.MINUTES.between(ticket.getDate(), ticket.getDatePriseEncharge());
            double delaiMaxPriseEnChargeMinutes = ticket.getSlaAssigne().getDelaiPriseEnChargeHeure() * 60.0;
            boolean respectePriseEnCharge = minutesEcouleesPriseEnCharge < delaiMaxPriseEnChargeMinutes;
            ticket.setSlaPriseEnChargeRespecte(respectePriseEnCharge);
        ...
        // 🛡️ AUDIT LOG : Prise en charge / Assignation
        String jsonAvant = String.format("{\"statut\":\"%s\",\"technicien\":\"Non assigné\"}", ancienStatut.name());
        String jsonApres = String.format("{\"statut\":\"EN_COURS\",\"technicien\":\"%s\",\"slaPriseEnChargeRespecte\":%b}",
                utilisateur.getEmail(), ticketUpdated.getSlaPriseEnChargeRespecte());

        auditService.enregistrerLogAvance(
                ModuleAudit.TICKET, ActionAudit.ASSIGN_TICKET, "Ticket", ticketUpdated.getIdTicket(),
                "Prise en charge et démarrage du ticket Réf: " + ticketUpdated.getReference() + " par le technicien " + utilisateur.getNom(),
                jsonAvant, jsonApres, NiveauAudit.INFO, true, utilisateur
        );

        historiqueService.tracer(ticket, utilisateur, "Statut", ancienStatut.name(), StatutTicket.En_Cours.name());
    ...
    @Transactional
    public Ticket resoudreTicket(Long idTicket, Long idUtilisateur, String noteResolution, Integer delaiResolution) {
        System.out.println("✅ [RÉSOUDRE TICKET] ID: " + idTicket);
        ...
        // 🛡️ AUDIT LOG : Résolution du Ticket (Alerte WARNING si le SLA global est cassé/dépassé !)
        NiveauAudit niveauSla = ticketUpdated.getSlaRespecte() ? NiveauAudit.INFO : NiveauAudit.WARNING;
        String jsonAvant = String.format("{\"statut\":\"%s\"}", ancienStatut.name());
        String jsonApres = String.format("{\"statut\":\"RESOLU\",\"delaiMinutes\":%d,\"slaGlobalRespecte\":%b}",
                delaiResolution, ticketUpdated.getSlaRespecte());

        auditService.enregistrerLogAvance(
                ModuleAudit.TICKET, ActionAudit.CLOSE_TICKET, "Ticket", ticketUpdated.getIdTicket(),
                "Résolution technique enregistrée pour le ticket Réf: " + ticketUpdated.getReference() + ". Note: " + noteResolution,
                jsonAvant, jsonApres, niveauSla, true, utilisateur
        );

        historiqueService.tracer(ticket, utilisateur, "Statut", ancienStatut.name(), StatutTicket.Resolu.name());
        historiqueService.tracer(ticket, utilisateur, "Délai Résolution", "N/A", delaiResolution + " minutes");
        historiqueService.tracer(ticket, utilisateur, "SLA Respecté", "N/A", ticket.getSlaRespecte() ? "OUI ✅" : "NON ❌");
    ...
    public Ticket cloturerTicket(Long idTicket, Long idUtilisateur, String roleUtilisateur) {
        ...
        boolean isAdmin = roleUtilisateur != null &&
                (roleUtilisateur.equalsIgnoreCase("ADMINISTRATEUR") || roleUtilisateur.equalsIgnoreCase("ADMIN"));
        ...
        // 🛡️ AUDIT LOG : Clôture administrative définitive
        String jsonAvant = String.format("{\"statut\":\"%s\"}", ancienStatut.name());
        String jsonApres = String.format("{\"statut\":\"CLOTURE\",\"dateCloture\":\"%s\"}", dateCloture.toString());

        auditService.enregistrerLogAvance(
                ModuleAudit.TICKET, ActionAudit.CLOSE_TICKET, "Ticket", ticketUpdated.getIdTicket(),
                "Clôture administrative définitive du ticket Réf: " + ticketUpdated.getReference(),
                jsonAvant, jsonApres, NiveauAudit.INFO, true, utilisateur
        );

        historiqueService.tracer(ticket, utilisateur, "Statut", ancienStatut.name(), StatutTicket.Cloture.name());
    ...
    public Ticket reouvrirTicket(Long idTicket, Long idUtilisateur) {
        ...
        // 🛡️ AUDIT LOG : Réouverture
        auditService.enregistrerLogAvance(
                ModuleAudit.TICKET, ActionAudit.REOPEN_TICKET, "Ticket", ticketMisAJour.getIdTicket(),
                "Réouverture du ticket Réf: " + ticketMisAJour.getReference() + " suite à une insatisfaction ou persistance du problème.",
                "{\"statut\":\"RESOLU\"}", "{\"statut\":\"EN_COURS\"}", NiveauAudit.WARNING, true, utilisateur
        );
    ...
    private void calculerSLA(Ticket ticket) {
        try {
            ...
            long heuresTotalesResolution = ChronoUnit.HOURS.between(ticket.getDate(), ticket.getDateResolution());
    ...
    public SLA obtenerSLA(Categorie categorie, Priorite priorite) {
        if (categorie == null || priorite == null) return null;
        return slaRepository.trouverSlaUnique(categorie.getIdCategorie(), priorite).orElse(null);
    }

    // ============================================================================
    // 1️⃣0️⃣ SUPPRIMER UN TICKET (Action sensible → Log Niveau WARNING)
    // ============================================================================
    @Transactional
    public void supprimerTicket(Long id) {
        System.out.println("🗑️ [SUPPRIMER] Ticket ID: " + id);
        ...
        // 🛡️ AUDIT LOG : Enregistrer l'action AVANT suppression définitive physique
        String jsonAvant = String.format("{\"id\":%d,\"reference\":\"%s\",\"titre\":\"%s\"}",
                ticket.getIdTicket(), ticket.getReference(), ticket.getTitre());

        auditService.enregistrerLogAvance(
                ModuleAudit.TICKET, ActionAudit.DELETE_TICKET, "Ticket", id,
                "Suppression définitive du ticket Réf: " + ticket.getReference() + " - Titre: " + ticket.getTitre(),
                jsonAvant, null, NiveauAudit.WARNING, true, ticket.getDemandeur()
        );

        ticketRepository.delete(ticket);
    }

    public List<Ticket> listerTous() { return ticketRepository.findAll(); }
    public List<Ticket> listerParDemandeur(Long idDemandeur) { return ticketRepository.findByDemandeur_Id(idDemandeur); }
    public List<Ticket> listerParGroupe(Long idGroupe) { return ticketRepository.findByGroupeAssigne_Id(idGroupe); }
    public List<Ticket> listerParStatut(StatutTicket statut) { return ticketRepository.findByStatut(statut); }
    public List<Ticket> listerParPriorite(Priorite priorite) { return ticketRepository.findByPriorite(priorite); }
    public List<Ticket> listerSLADepasses() { return ticketRepository.findBySlaRespecte(false); }
    public Ticket getTicketById(Long id) { return ticketRepository.findById(id).orElseThrow(() -> new RuntimeException("Ticket non trouvé")); }
    ...
    public TicketStatisticsDTO obtenirStatistiques() {
        ...
        return TicketStatisticsDTO.builder()
                .totalTickets(totalTickets).ticketsOuverts(ticketsOuverts).ticketsEnCours(ticketsEnCours)
                .ticketsResolus(ticketsResolus).ticketsClotures(ticketsClotures).slaDepassesCount(slaDepassesCount)
                .build();
    }

    public SlaStatisticsDTO obtenirStatistiquesSLA() {
        long total = ticketRepository.countBySlaAssigneIsNotNull();
        if (total == 0) return new SlaStatisticsDTO();
        ...
        for (Object[] row : resultatsParPriorite) {
            if (row[0] != null) {
                String priorite = row[0].toString();
                Double taux = row[1] != null ? ((Number) row[1]).doubleValue() : 0.0;
                repartitionPriorite.put(priorite, taux);
            }
        }

        return SlaStatisticsDTO.builder()
                .totalTicketsAvecSla(total).slaGlobauxRespectes(respectes).slaGlobauxDepasses(depasses)
                .tauxReussiteGlobal(Math.round(tauxGlobal * 100.0) / 100.0)
                .reussitePriseEnCharge(okPriseEnCharge).echecPriseEnCharge(koPriseEnCharge)
                .reussiteResolution(okResolution).echecResolution(koResolution)
                .tauxReussiteParPriorite(repartitionPriorite)
                .build();
    }
    public SLA obtenirSLA(Categorie categorie, Priorite priorite) {
